<%@ page
	language="java"
	contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<!-- Main sidebar -->
                <div class="sidebar sidebar-main sidebar-default">
                    <div class="sidebar-content">
                        <!-- Main navigation -->
                        <div class="sidebar-category sidebar-category-visible">
                            <div class="category-content sidebar-user">
                                <div class="media">
                                    <a href="#" class="media-left">
                                        <img src="../addon/assets/images/placeholder.jpg" class="img-circle img-sm" alt="">
                                    </a>
                                    <div class="media-body">
                                        <span class="media-heading text-semibold">Victoria Baker</span>
                                        <div class="text-size-mini text-muted">
                                            ผู้จัดการ
</div>
                                    </div>
                                </div>
                            </div>
                            <div class="category-content no-padding"> 
                                <ul class="navigation navigation-main navigation-accordion"> 
                                    <!-- Main -->                                     
                                    <li class="navigation-header"> 
                                        <span>Main</span> 
                                        <i class="icon-menu" title="Main pages"></i> 
                                    </li>                                     
                                    <li class="active"> 
                                        <a href="index.jsp"><i class="icon-home4"></i> <span>แผงควบคุม</span></a> 
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-file-text-o"></i> <span>ใบรับรายการ</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="../_SamePage/create_order.jsp">สร้างใบรับรายการ</a> 
                                            </li>                                             
                                            <li> 
                                                <a href="../_SamePage/list_order.jsp">ใบรับรายการทั้งหมด</a> 
                                            </li>
                                        </ul>                                         
                                    </li>
                                    <li> 
                                        <a href="#"><i class="fa fa-file-text-o"></i> <span>ใบสั่งปฏิบัติงาน</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="../_SamePage/create_shipment.jsp">สร้างใบสั่งปฏิบัติงาน</a> 
                                            </li>                                             
                                            <li> 
                                                <a href="../_SamePage/list_shipment.jsp">ใบสั่งปฏิบัติงานทั้งหมด</a> 
                                            </li>
                                        </ul>                                         
                                    </li>
                                    <li> 
                                        <a href="#"><i class="fa fa-money"></i><span>ลูกหนี้</span></a> 
                                        <ul>
                                            <li> 
                                                <a href="../_SamePage/list_debtor.jsp">ลูกหนี้ทั้งหมด</a> 
                                            </li>                                             
                                            <li> 
                                                <a href="../_SamePage/list_payment.jsp">พิมพ์ใบเสร็จ</a> 
                                            </li>                                             
                                            <li> 
                                                <a href="../_SamePage/payment.jsp">ชำระเงิน</a> 
                                            </li>                                             
                                        </ul>
                                    </li>  
                                    <li> 
                                        <a href="#"><i class="fa fa-user"></i> <span>ผู้ว่าจ้าง</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="../_SamePage/list_customer.jsp">รายการผู้ว่าจ้าง</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-users"></i> <span>พนักงาน</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="list_staff.jsp">รายการพนักงาน</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-dashboard"></i> <span>คนขับรถ</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="list_driver.jsp">รายการคนขับรถ</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-thumbs-o-up"></i> <span>คนประสานงาน</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="list_coordinator.jsp">รายการคนประสานงาน</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-truck"></i> <span>รถใช้งาน</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="list_car.jsp">รายการรถใช้งาน</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <li> 
                                        <a href="#"><i class="fa fa-warning"></i> <span>ประวัติการเข้าใช้งาน</span></a> 
                                        <ul> 
                                            <li> 
                                                <a href="../_SamePage/history_login.jsp">รายการประวัติการเข้าใช้งาน</a> 
                                            </li>                                             
                                        </ul>                                         
                                    </li>                                     
                                    <!-- /main -->                                     
                                </ul>                                 
                            </div>                             
                        </div>
                        <!-- /main navigation -->
                    </div>
                </div>
                <!-- /main sidebar -->

